-- Code completion and git blame
vim.api.nvim_set_hl(0, "Pmenu", { fg = "#cccccc", bg = "#2d2d2d" })
vim.api.nvim_set_hl(0, "PmenuSel", { fg = "#ffffff", bg = "#515151" })
vim.api.nvim_set_hl(0, "PmenuSbar", { bg = "#6699cc" })
vim.api.nvim_set_hl(0, "PmenuThumb", { bg = "#515151" })
vim.api.nvim_set_hl(0, "CmpDocNormal", { bg = "#2d2d2d" })
vim.api.nvim_set_hl(0, "NormalFloat", { bg = "#2d2d2d" })
vim.api.nvim_set_hl(0, "FloatBorder", { bg = "#2d2d2d" })
vim.api.nvim_set_hl(0, "CmpItemKindCopilot", { fg = "#66cccc" })

-- Notification
vim.api.nvim_set_hl(0, "NotifyERRORBorder", { fg = "#f2777a" })
vim.api.nvim_set_hl(0, "NotifyWARNBorder", { fg = "#f99157" })
vim.api.nvim_set_hl(0, "NotifyHINTBorder", { fg = "#ffcc66" })
vim.api.nvim_set_hl(0, "NotifyINFOBorder", { fg = "#99cc99" })
vim.api.nvim_set_hl(0, "NotifyERRORIcon", { fg = "#f2777a" })
vim.api.nvim_set_hl(0, "NotifyWARNTIcon", { fg = "#f99157" })
vim.api.nvim_set_hl(0, "NotifyHINTIcon", { fg = "#ffcc66" })
vim.api.nvim_set_hl(0, "NotifyINFOIcon", { fg = "#99cc99" })
vim.api.nvim_set_hl(0, "NotifyERRORTitle", { fg = "#f2777a" })
vim.api.nvim_set_hl(0, "NotifyWARNTitle", { fg = "#f99157" })
vim.api.nvim_set_hl(0, "NotifyHINTTitle", { fg = "#ffcc66" })
vim.api.nvim_set_hl(0, "NotifyINFOTitle", { fg = "#99cc99" })

-- Code editor
vim.api.nvim_set_hl(0, "Delimiter", { fg = "#ffffff" })
vim.api.nvim_set_hl(0, "@string.special", { fg = "#f99157" })
vim.api.nvim_set_hl(0, "@string.regexp", { fg = "#99cc99" })
vim.api.nvim_set_hl(0, "@string.escape", { fg = "#f99157" })
vim.api.nvim_set_hl(0, "@variable", { fg = "#f2777a" })

-- Ruby icons
vim.api.nvim_set_hl(0, "DevIconRb", { fg = "#e8274b" })
vim.api.nvim_set_hl(0, "DevIconErb", { fg = "#e8274b" })
vim.api.nvim_set_hl(0, "DevIconConfigRu", { fg = "#e8274b" })
vim.api.nvim_set_hl(0, "DevIconGemfile", { fg = "#e8274b" })
vim.api.nvim_set_hl(0, "DevIconRake", { fg = "#e8274b" })
vim.api.nvim_set_hl(0, "DevIconRakefile", { fg = "#e8274b" })
